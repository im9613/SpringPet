    ...
    <div id="board-list">
        <div class="container">
            <table class="board-table">
                ...
                <c:if test="${not empty reviewList}">
				    <c:forEach items="${reviewList}" var="AdoptReviewListDTO">
				        <tr>
		           			...
		           			<th><a href="/pet/reviewdetail.do?seq=${AdoptReviewListDTO.seq}">${AdoptReviewListDTO.title}</a></th>
		 	...
		 	<button class="btn btn-primary rounded-pill d-inline-flex flex-shrink-0 py-2 px-4"  style="flex-direction: column;" type="submit" onclick="location.href='/pet/reviewadd.do';">글쓰기</button>
        </div>
    </div>
...
<div class="page">
    	<c:if test="${pageNum > 1}">
    		<a href="/pet/reviewlist.do?pageNum=1"><<</a>
    		<a href="/pet/reviewlist.do?pageNum=${pageNum - 1}"><</a>
    	</c:if>
    	
    	<c:forEach begin="1" end="${totalPages}" var="i">
     		<a href="/pet/reviewlist.do?pageNum=${i}" class="<c:if test='${i == pageNum}'>active</c:if>">${i}</a>
    	</c:forEach>
    	
   	  	<c:if test="${pageNum < totalPages}">
            <a href="/pet/reviewlist.do?pageNum=${pageNum + 1}">></a>
            <a href="/pet/reviewlist.do?pageNum=${totalPages}">>></a>
        </c:if>
